...
describe("exact persistence engine and safety backup creation unit test", () => {
  it("implements exact state tracking, history sorting, and safety backup creation matching server db behavior", async () => {
    const table: Array<{
      id: number;
    ...
    const op1 = await createOperation({ operationType: "safety_backup", status: "started", stage: "Safety backup в процессе", totalRecords: 100 });
    expect(op1).toBe(500);

    await updateOperation(op1, { status: "success", progress: 100, processedRecords: 100, stage: "Safety backup создан" });

    // Simulate import operation
    const op2 = await createOperation({ operationType: "import", status: "started", fileName: "export.json", stage: "Импорт", totalRecords: 200 });
    expect(op2).toBe(op1 + 1);

    await updateOperation(op2, { status: "success", progress: 100, processedRecords: 200, stage: "Импорт завершен" });
    ...
    expect(calculateBackupProgress(100, 200, 0, 100)).toBe(50);
  });
});
